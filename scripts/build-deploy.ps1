...
if (-not (Test-Path $standaloneNext)) {
    Write-Error "Missing .next\standalone\.next - check output: standalone in next.config.ts"
}
...
Write-Host ""
Write-Host "Fix chunk failed on server:" -ForegroundColor Yellow
Write-Host "  1. SSH delete old .next: rm -rf /var/www/admindev.yocostays.com/.next"
Write-Host "  2. WinSCP upload ALL files inside deploy folder"
Write-Host "  3. SSH: pm2 restart yoco-admin-dev"
Write-Host "  4. Browser hard refresh Ctrl+Shift+R"
